...
`PASS`. Tier 3 evidence is publication-required by default, so a missing,
skipped, or incomplete live evaluation produces `INCOMPLETE` rather than a
publication recommendation. A catalog may explicitly persist
`benchmark_policy.tier3_required = false`; the card discloses that exception
instead of implying that Tier 3 ran.
